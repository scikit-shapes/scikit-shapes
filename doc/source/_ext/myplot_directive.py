...
def _option_context(arg):
    if arg in [None, 'reset', 'close-figs']:
        return arg
    msg = "Argument should be None or 'reset' or 'close-figs'"
    raise ValueError(msg)
...
def mark_plot_labels(_, document):
    """
...
class PlotDirective(Directive):
    ...
    def run(self):
        ...
        try:
            ...
            raise self.error(str(e))  # noqa: B904
...
def setup(app):
    ...
    app.add_config_value('myplot_template', None, True)
    app.add_config_value('myplot_srcset', [], True)
...
class ImageFile:
    ...
    def filename(self, format):
        return os.path.join(self.dirname, f"{self.basename}.{format}")  # noqa: PTH118
...
def out_of_date(original, derived, includes=None):
    ...
    *derived* and *original* are full paths, and *includes* is optionally a
    ...
    """
    if not os.path.exists(derived):  # noqa: PTH110
        return True
    ...
    def out_of_date_one(original, derived_mtime):
        return (os.path.exists(original) and  # noqa: PTH110
                derived_mtime < os.stat(original).st_mtime)  # noqa: PTH116

    derived_mtime = os.stat(derived).st_mtime  # noqa: PTH116
...
def _run_code(code, code_path, ns=None, function_name=None):
    ...
    if setup.config.myplot_working_directory is not None:
        try:
            os.chdir(setup.config.myplot_working_directory)
        ...
        dirname = os.path.abspath(os.path.dirname(code_path))  # noqa: PTH100, PTH120
        os.chdir(dirname)

    src_path = str(Path(setup.confdir).resolve())
    with cbook._setattr_cm(
            sys,
            argv=[code_path],
            path=[src_path] + sys.path), \
            contextlib.redirect_stdout(StringIO()):  # noqa: RUF005
        try:
            if ns is None:
                ns = {}
            if not ns:
                if setup.config.myplot_pre_code is None:
                    exec('import numpy as np\n'
                         'from matplotlib import pyplot as plt\n', ns)
                else:
                    exec(str(setup.config.myplot_pre_code), ns)
                source_root = Path(setup.confdir).resolve()
            ...
            with cbook._setattr_cm(FigureManagerBase, show=lambda self: None):  # noqa: ARG005
                exec(code, ns)
...
def get_plot_formats(config):
    ...
    formats = []
    plot_formats = config.myplot_formats
...
def _parse_srcset(entries):
    ...
    for entry in entries:
        ...
        if len(entry) >= 2:
            ...
            msg = f'srcset argument {entry!r} is invalid.'
...
def render_figures(code, code_path, output_dir, output_base, context,
    ...
    *output_base*
    ...
    for format, dpi in formats:  # noqa: B007
    ...
    if context_reset:
        clear_state(config.myplot_rcparams)
        plot_context.clear()
    ...
    for i, code_piece in enumerate(code_pieces):
        ...
        if not context or config.myplot_apply_rcparams:
            clear_state(config.myplot_rcparams, close_figs)
        elif close_figs:
            plt.close('all')
        ...
        if not _pylab_helpers.Gcf.get_all_fig_managers() and 'fig' in ns:
            fig = ns['fig']
            try:
                import matplotlib.figure
                if isinstance(fig, matplotlib.figure.Figure):
                    figman = plt.figure(fig.number).canvas.manager
            except Exception:
                figman = None

            if figman:
                _pylab_helpers.Gcf.set_active(figman)
        ...
        for j, figman in enumerate(fig_managers):
            ...
            for fmt, dpi in formats:
                try:
                    ...
                    if fmt == formats[0][0] and config.myplot_srcset:
                        # save a 2x, 3x etc version of the default...
                        srcset = _parse_srcset(config.myplot_srcset)
    ...
    if not context or config.myplot_apply_rcparams:
        clear_state(config.myplot_rcparams, close=not context)
...
def run(arguments, content, options, state_machine, state, lineno):
    ...
    if config.myplot_srcset and setup.app.builder.name == 'singlehtml':
        msg = 'plot_srcset option not compatible with single HTML writer'
    ...
    options.setdefault('include-source', config.myplot_include_source)
    options.setdefault('show-source-link', config.myplot_html_show_source_link)
    ...
    if len(arguments):
        if not config.myplot_basedir:
            source_file_name = os.path.join(setup.app.builder.srcdir,  # noqa: PTH118
                                            directives.uri(arguments[0]))
        else:
            source_file_name = os.path.join(setup.confdir, config.myplot_basedir,  # noqa: PTH118
                                            directives.uri(arguments[0]))
    ...
    build_dir = os.path.join(os.path.dirname(setup.app.doctreedir),  # noqa: PTH118, PTH120
                             'plot_directive',
                             source_rel_dir)
    ...
    try:
        source_file_includes = [os.path.join(os.getcwd(), t[0])  # noqa: PTH109, PTH118
        ...
        possible_sources = {os.path.join(setup.confdir, t[0])  # noqa: PTH118
    ...
    if caption and config.myplot_srcset:
    ...
    for j, (code_piece, images) in enumerate(results):
        ...
        if j == 0 and options['show-source-link']:
            ...
            src_name = None
        if config.myplot_srcset:
            srcset = [*_parse_srcset(config.myplot_srcset).values()]
            template = TEMPLATE_SRCSET
        else:
            srcset = None
            template = TEMPLATE

        result = jinja2.Template(config.myplot_template or template).render(
            ...
            html_show_formats=config.myplot_html_show_formats and len(images),
            caption=caption)
