...
Lx = torch.stack(
    [2 * x, zero, zero, y, z, zero, ones, zero, zero, zero], dim=-1
)
Ly = torch.stack(
    [zero, 2 * y, zero, x, zero, z, zero, ones, zero, zero], dim=-1
)
Lz = torch.stack(
    [zero, zero, 2 * z, zero, x, y, zero, zero, ones, zero], dim=-1
)
...
N = (
    Lx.transpose(1, 2) @ Lx + Ly.transpose(1, 2) @ Ly + Lz.transpose(1, 2) @ Lz
) / n
...
eigenvalues, eigenvectors = torch.lobpcg(
    A=M, B=N, k=1, largest=False, tol=1e-5
)
