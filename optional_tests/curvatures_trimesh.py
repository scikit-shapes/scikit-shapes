...
def test_sphere_trimesh():
    ...
    def plot_histogram_curvatures(curvs_1, curvs_2) :
        ...
        for curv in [curvs_1, curvs_2]:
            print('Mean:', curv.mean(), 'Std:', curv.std(), 'Min:', curv.min(), 'Max:', curv.max())
    ...
    if True:
        shape_tri = trimesh.creation.icosphere(radius = 5, subdivisions=5)
    elif True:
        shape_tri = trimesh.creation.torus(major_radius = 1, minor_radius = .25)
    elif False:
        shape_tri = trimesh.creation.capsule(radius = 1, height = .2)
    else:
        shape_pv = pv.ParametricRandomHills(u_res=30, v_res=30, w_res=30).triangulate()
        diam = shape_pv.points.max() - shape_pv.points.min()


        shape_tri = trimesh.Trimesh(vertices=shape_pv.points / diam, faces=shape_pv.faces.reshape(-1, 4)[:, 1:])
        print(shape_tri)
        print(shape_tri.vertices.max(), shape_tri.vertices.min())


    shape_sks = sks.PolyData(points=shape_tri.vertices, triangles = shape_tri.faces)
    print(shape_sks.n_points)

    SCALE = 2
    mean_curvs, gauss_curvs = shape_sks.point_mean_gauss_curvatures(scale=SCALE)
    kmax, kmin = shape_sks.point_principal_curvatures(scale=SCALE)
    ...
    # trimesh
    mean_curvs_t, gauss_curvs_t = compute_mean_gauss_curvatures(shape_tri,
                                                            scale = 2 * SCALE,
                                                            method = 'trimesh')
    ...
    sns.jointplot(data, x="mean_curvs_t", y="gauss_curvs_t", kind="kde")
    plt.show()
    ...
    pl.subplot(1, 1)
    pl.add_mesh(shape_sks.points.numpy(), scalars=mean_curvs_t)
    pl.subplot(1, 2)
    pl.add_mesh(shape_sks.points.numpy(), scalars=gauss_curvs_t)
